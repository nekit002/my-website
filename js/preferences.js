...
const translations = {
  ...
  'All levels':'Все уровни','Beginner':'Начальный','Intermediate':'Средний','Advanced':'Продвинутый',
  'Any rating':'Любой рейтинг','4.5 and above':'От 4,5','Try again':'Повторить',
  'Reset all':'Сбросить всё','No courses found':'Курсы не найдены','Previous':'Назад','Next':'Далее','Per page':'На странице',
  ...
  'Webflow Basics':'Основы Webflow','Webflow Interactions Masterclass':'Мастер-класс по интерактивности Webflow',
  'Advanced CMS and Integrations':'Продвинутые CMS и интеграции','Join us':'Присоединяйтесь',
  'Join more than 17,000+ students all over the world':'Присоединяйтесь к более чем 17 000 студентов по всему миру',
...
function translateDynamic(phrase) {
  ...
  if ((match = phrase.match(/^(.+) added to cart\.$/))) return match[1] + ' добавлен в корзину.';
...
function translateNode(node) {
  ...
  const target = language === 'ru' ? (translations[phrase] || translateDynamic(phrase)) : phrase;
  if (!target) return;
...
function translateAll() {
  ...
  for (const element of document.querySelectorAll('[placeholder], [aria-label]')) {
    for (const attribute of ['placeholder', 'aria-label']) {
      ...
      element.setAttribute(attribute, language === 'ru' ? translations[source] || source : source);
    }
  }
  document.title = language === 'ru' ? translations[originalTitle] || originalTitle : originalTitle;
  document.documentElement.lang = language;
  document.documentElement.dataset.theme = theme;
  for (const img of document.querySelectorAll('[data-theme-light][data-theme-dark]')) img.src = img.dataset[theme === 'dark' ? 'themeDark' : 'themeLight'];
